...
目标是在独立测试集上，相对对称 Set-MST，使第二阶段完整计算耗时总体减少至少 20%，最终配送成本总体增加不超过 10%，同时报告逐实例超限比例和最坏退化。运行入口见 [阶段 C 服务器指南](partition_repair_stage_c_server_guide.md)。历史诊断表中的 5% 仍指对应历史分析的实际预算。

推荐研究主线：**生成不同修复力度的划分候选，预测相对初始划分的成本变化和节时收益，然后一次选择合适的划分。** 第三阶段保持为固定配送评价器。强化学习作为有证据支持多步决策价值后的扩展。

**1. 代码与历史证据边界**

历史代码为 `260831学习型尝试`，提交 `168914d`。研究基线提交为 `34ce513`。这两个历史版本的 `src/fstsp.py`、`utils.py`、`problem.py`、`config.py`、`experiments.py`、`experiment_results.py` 无差异；旧分支主要增加独立的学习和评估模块。当前可执行选择器位于 `src/partition_repair`。
...
逐实例附表至少给出成本增加超过 10% 的数量、比例、最大恶化、时间变慢的数量和最差变慢；超时和无可行解回退不得从主结果中剔除。置信区间按完整实例配对重采样，并保留原有规模分层；邻域组和重复计时不是新增独立实例。
...
监督模型分别预测成本差和节时，采用浅层树回归与带标准化的岭回归对照。训练、早停和阈值选择按完整实例隔离，不使用估计器内部按行随机划分替代实例划分。开发诊断采用外层留出与训练部分内部校准，正式策略在独立验证实例上校准。

总体成本约束不必强制每个候选都通过单实例 10% 门槛。在验证集上选择固定权衡参数 λ，使用 `预测节时 − λ × 预测成本增量` 选候选，并保留 `stay`。输出 8%/9%/10% 校准曲线；λ、修复力度和拒绝规则全部在最终测试前冻结，不在测试集事后分配成本额度。
...
| D：独立验收 | 每规模 30 个全新测试实例，共 90 个；固定方法，优化器种子匹配，方法执行顺序交替；噪声敏感病例做重复计时 | 总体指标、分规模和逐实例附表、配对置信区间 | 总体成本增加不超过 10%，完整 T₂ 减少至少 20%；如区间仍跨越门槛，结论保留不确定性 |
...
核心模块职责如下，完整训练、校准、清单准备和测试入口见 [阶段 C 实施说明](partition_repair_stage_c_plan.md) 与 [服务器指南](partition_repair_stage_c_server_guide.md)：
...
当前可使用已有阶段 B 标签执行 CPU 分组诊断；按规模调权与学习选择共用固定候选集合。完成开发诊断后，按批增加训练与验证实例，再进行冻结策略的实际复测和独立测试。
